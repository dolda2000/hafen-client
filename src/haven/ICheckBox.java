...
public class ICheckBox extends ACheckBox {
	...
	public Widget create(UI ui, Object[] args) {
	    Tex up = Loading.waitfor(ui.sess.getres(args[0])).flayer(Resource.imgc).tex();
	    Tex down = Loading.waitfor(ui.sess.getres(args[1])).flayer(Resource.imgc).tex();
	    Tex hoverup = (args.length > 2) ? Loading.waitfor(ui.sess.getres(args[1])).flayer(Resource.imgc).tex() : up;
	    Tex hoverdown = (args.length > 3) ? Loading.waitfor(ui.sess.getres(args[1])).flayer(Resource.imgc).tex() : down;
