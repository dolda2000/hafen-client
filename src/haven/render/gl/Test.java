...
public class Test implements GLEventListener, KeyListener {
...

	{
	    ...
	    g.draw(pipe, new Model(Model.Mode.TRIANGLES,
				   new VertexArray(fmt, 3, new VertexArray.Buffer(data.length, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))),
				   null));
	}

	{
	    ...
	    g.draw(pipe, new Model(Model.Mode.TRIANGLE_STRIP,
				   new VertexArray(fmt, 4, new VertexArray.Buffer(data.length, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))),
				   null));
	}

	{
	    ...
	    g.draw(pipe, new Model(Model.Mode.TRIANGLE_STRIP,
				   new VertexArray(fmt, 4, new VertexArray.Buffer(data.length * 4, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))),
				   null));
	}
    }
